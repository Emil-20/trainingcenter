...
def savetmreplaytrnr(request, id):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            vars.save()
            msg_success = "Replied successfully"
            return render(request,'software_training/training/manager/manager_trainer_unsolvedissue.html', {'mem': mem,'msg_success':msg_success})
...
def savetmreplytrns(request, id):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            msg_success = "Replied successfully"
            vars.save()
            return render(request,'software_training/training/manager/manager_trainee_unsolvedissue.html', {'mem': mem,'msg_success':msg_success})
...
def manager_report_issue(request):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            msg_success = "Issuse Reported Succesfully"
            vars.save()
            return render(request, 'software_training/training/manager/manager_report_issue.html', {'mem': mem,'msg_success':msg_success})
...
def manager_accountant_attendance(request):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
        ...
        vars = user_registration.objects.filter(designation_id=des.id)        
        return render(request, 'software_training/training/manager/manager_accountant_attendance.html',{'mem':mem, 'vars':vars})
    else:
        return redirect('/')  

def manager_accountant_attendance_table(request):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
        ...
        if request.method == 'POST':
            ...
            user = request.POST['accounts']
            attend=attendance.objects.filter(attendance_date__gte=start,attendance_date__lte=end,attendance_user_id=user)
        return render(request, 'software_training/training/manager/manager_accountant_attendance_table.html',{'mem':mem,'vars':attend})
    else:
        return redirect('/')  
...
def manager_applyleavsub(request):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            msg_success = "Leave Applied Succsesfully"   
            vars.save()
            return render(request,'software_training/training/manager/manager_applyleavsub.html', {'mem': mem,'msg_success':msg_success})
...
def approvedstatus(request,id):
    ...
    msg_success = "LeaveApproved Succesfully"
    a.save()
    return render(request,'software_training/training/manager/manager_trainers_leavelist.html',{'msg_success':msg_success,'mem': mem})
...
def Leave_rejected(request,id):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            msg_success = "Leave Rejected Succsesfully" 
            vars.save()
        return render(request,'software_training/training/manager/manager_trainers_leavelist.html',{'msg_success':msg_success,'mem': mem})
    else:
        return redirect('/')
...
def manager_accountant_leave(request):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
        ...
        mem = user_registration.objects.filter(designation_id=m_designation_id).filter(fullname=m_fullname) 
        return render(request, 'software_training/training/manager/manager_accountant_leave.html',{'mem':mem})
    else:
        return redirect('/')


def manager_accountant_leavelist(request):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
        ...
        cut = leave.objects.filter(leave_designation_id=des.id).filter(leave_leaveapproved_status=0).order_by('-id') 
        context = {'cut': cut, 'vars': vars, 'mem': mem}
        return render(request,'software_training/training/manager/manager_accountant_leavelist.html', context)
    else:
        return redirect('/')

def approvedstatus_accountant(request,id):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
            ...
            m_fullname = "dummy"   
    mem = user_registration.objects.filter(id=m_id).filter(fullname=m_fullname) 
    a=leave.objects.get(id=id)
    a.leave_leaveapproved_status=1
    msg_success = "Leave Approved Succsesfully"
    a.save()
    return render(request,'software_training/training/manager/manager_accountant_leavelist.html',{'msg_success':msg_success,'mem': mem})

def Leave_rejected_accountant(request,id):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
...
def manager_accountant_leavestatus(request):
    if 'm_id' in request.session:
        if request.session.has_key('m_id'):
            m_id = request.session['m_id']
        if request.session.has_key('m_fullname'):
        ...
        des = designation.objects.get(designation_name='accounts')
        n = leave.objects.filter(leave_designation_id=des.id).order_by('-id') 
        return render(request, 'software_training/training/manager/manager_accountant_leavestatus.html', {'mem': mem ,'n': n})
    else:
        return redirect('/')
...
def approvedstatus_trainee(request,id):
    ...
    msg_success = "Leave Approved Succsesfully" 
    a.save()
    return render(request,'software_training/training/manager/manager_trainee_leavelist.html',{'msg_success':msg_success,'mem': mem})
...
def manager_newteamadd(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            msg_success = "New Team Created Sucessfully"
            user.save()
            return render(request,'software_training/training/manager/manager_new_teamcreate.html',{'mem':mem,'msg_success':msg_success, 'var': var})
...
def manager_teamupdate(request,id):
    ...
    if request.method == 'POST':
        ...
        abc.save()
        msg_info = "Succesfully Updated"
        return render(request, 'software_training/training/manager/manager_new_team.html',{'msg_info':msg_info,'mem': mem,})
    else:
        pass
...
def manager_teamdelete(request,id):
    ...
    var.delete()
    msg_success = "Succesfully Deleted"
    return render(request, 'software_training/training/manager/manager_new_team.html',{'msg_success':msg_success,'mem': mem,})
...
def manager_newtraineeesteam(request,id):
    if 'm_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            msg_success = "Submited Succesfully"
            register.save()
            return render(request, 'software_training/training/manager/manager_newtrainees.html', {'memm': memm, 'des': des, 'team': team,'msg_success':msg_success ,'mem':mem})
        ...
        if request.method == 'POST':
            ...
            if oldps != newps:
                if newps == cmps:
                    ...
                    abc.save()
                    msg_success = "Password Change Succesfully"
                    return render(request, 'software_training/training/manager/manager_Dashboard.html', {'mem': mem,'msg_success':msg_success})
            elif oldps == newps:
...
def manager_imagechange(request,id): 
    ...
    if request.method == 'POST':
        ...
        abc.save()
        msg_success = "Profile Photo Updated successfully"        
        return render(request, 'software_training/training/manager/manager_accountedit.html', {'mem': mem,'msg_success':msg_success})
    return render(request, 'software_training/training/manager/manager_accountedit.html', {'mem': mem,})
...
def trainerimagechange(request,id):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
          ...
          abc.photo = request.FILES['filename']
          abc.save()
          msg_success = "Image Change successfully"
          z = user_registration.objects.filter(id=tr_id)
        return render(request,'software_training/training/trainer/trainer_imagechange.html',{'z':z,'msg_success':msg_success})
    else:
        return redirect('/')
        ...
        if request.method=='POST':
            ...
            if oldps != newps:
                if newps == cmps:
                    abc.password = request.POST.get('confirmPassword')
                    abc.save()
                    msg_success = "Password Change successfully"
                    return render(request, 'software_training/training/trainer/trainer_passwordchange.html', {'z':z,'msg_success':msg_success})
...
def trainer_addtopic(request):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            mem.save()
            msg_success = "Topic Submitted"
            return render(request,'software_training/training/trainer/trainer_addtopic.html',{'crt':crt,'mem': mem,'z':z,'msg_success':msg_success})
...
def trainer_currentperform(request,id):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            mem.save()
            msg_success = "Performance Submitted"
            return render(request,'software_training/training/trainer/trainer_current_perform.html',{'mem': mem, 'z': z,'msg_success':msg_success})
...
def trainer_currentattenadd(request,id):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            atten.save()
            msg_success = "Attendance Submitted"
            return render(request,'software_training/training/trainer/trainer_current_atten_add.html',{'mem': mem, 'atten': atten, 'z': z,'msg_success':msg_success})
...
def trainer_givetask(request,id) :
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            vars.save()
            msg_success = "Task Submitted"
            return render(request, 'software_training/training/trainer/trainer_givetask.html', {'z': z, 'var': var,'msg_success':msg_success})
        else:
...
def trainer_applyleave_form(request):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            return render(request, 'software_training/training/trainer/trainer_applyleave.html', {'z': z,'msg_success': msg_success})
...
def trainer_reportissue_form(request):
    if 'tr_id' in request.session:
        ...
        if request.method == "POST":
            ...
            mem.save()
            msg_success ="issue reported successfully"
            return render(request, 'software_training/training/trainer/trainer_reportissue_form.html', {'z': z,'msg_success':msg_success})
...
def trainer_attendance_trainees_addattendance(request):
    if 'tr_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            return render(request, 'software_training/training/trainer/trainer_attendance_trainees_addattendance.html',{'z':z,'vars':vars,'msg_success':msg_success})
...
def trainer_appliedleave(request):
    if 'tr_id' in request.session:
        ...
        return render(request, 'software_training/training/trainer/trainer_appliedleave.html',{'tm': tm, 'z': z})
    else:
        return redirect('/')
...
def trainee_task_details(request,id):
    if 'te_id' in request.session:
        ...
        if request.method=="POST":
            ...
            return render(request,'software_training/training/trainee/trainee_task_details.html',{'z':z,'mem':mem,'msg_success':msg_success})
...
def save_trainee_review(request,id):
    if 'te_id' in request.session:
        ...
        vars.topic_status = 1
        vars.save()
        msg_success = "Review Submitted"
        return render(request,'software_training/training/trainee/trainee_currentTopic.html',{'z':z,'msg_success':msg_success})
    else:
        return redirect('/')  
...
def trainee_report_reported(request):
    if 'te_id' in request.session:
        ...
        if request.session.has_key('te_designation_id'):
            ...
            te_id = "dummy"
        z=user_registration.objects.filter(id=te_id,designation_id=te_designation_id)
        ...
        if request.method == 'POST':
            ...
            var.save()
            msg_success="Issue Reported Successfully"
            return render(request, 'software_training/training/trainee/trainee_report_issue.html',{'z':z,'msg_success':msg_success})
...
def trainee_report_issue(request):
    if 'te_id' in request.session:
        ...
        team = create_team.objects.get(id=te_team_id)
        mem3= user_registration.objects.filter(team_id=team,designation_id=des1.id)
        return render(request, 'software_training/training/trainee/trainee_report_issue.html',{'z':z,'mem':mem,'mem1':mem1,'mem3':mem3})
    else:
        return redirect('/')
...
def trainee_applyleave_form(request):
    if 'te_id' in request.session:
        ...
        if request.method=="POST":
            ...
            le.leave_user=user
            le.save()
            msg_success="Leave Applied Successfully"
            return render(request, 'software_training/training/trainee/trainee_applyleave_form.html',{'mem': mem,'z':z,'msg_success':msg_success}) 
...
def trainee_payment_addpayment(request):
    if 'te_id' in request.session:
        ...
        if request.method=="POST":
            ...
            return render(request,'software_training/training/trainee/trainee_payment_addpayment.html',{'z':z,'msg_success':msg_success})
...
def imagechange_trainees(request,id):
    if 'te_id' in request.session:
        ...
        if request.method=="POST":
            ...
            mem.save()
            msg_success="Photo Updated Successfully"
            return render(request,'software_training/training/trainee/trainees_account.html',{'z':z,'msg_success':msg_success})   
    ...
    if 'te_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            if oldps != newps:
                ...
                messages.info(request, 'Incorrect password same')                
            return render(request, 'software_training/training/trainee/trainees_chpasswd.html', {'z': z,'msg_success':msg_success})   
...
def Admin_categorieslist(request):
    ...
    use=category.objects.all()
    return render(request,'software_training/training/admin/admin_categorieslist.html', {'mem': mem,'var':var,'use':use}) 




def Admin_courselist(request,id):
    mem = User.objects.all()
    z = designation.objects.get(designation_name='Admin')
    var = user_registration.objects.filter(designation_id=z.id)
    ...
    use= course.objects.filter(course_category=a)
    return render(request,'software_training/training/admin/admin_courselist.html', {'mem': mem,'var':var,'use':use}) 

def Admin_coursedetails(request,id):
    mem = User.objects.all()
    z = designation.objects.get(designation_name='Admin')
    var = user_registration.objects.filter(designation_id=z.id)

    
    use= course.objects.get(id=id)
    print(use)
    return render(request,'software_training/training/admin/admin_coursedetails.html', {'mem': mem,'var':var,'use':use}) 
        ...
        if request.method == 'POST':
            ...
            if oldps != newps:
                if newps == cmps:
                    ...
                    abc.save()
                    msg_success="password updated successfully"
                    return render(request, 'software_training/training/account/changepassword_accounts.html', {'mem': mem,'msg_success':msg_success})
            elif oldps == newps:
                 msg_warning="old and new password same"
                 return render(request, 'software_training/training/account/changepassword_accounts.html', {'mem': mem,'msg_warning':msg_warning})
            else:
                 msg_error="password not matching"
                 return render(request, 'software_training/training/account/changepassword_accounts.html', {'mem': mem,'msg_error':msg_error})
...
def accounts_payment_salary(request,id):
    if 'acc_id' in request.session:
        ...
        if request.method == "POST":
            ...
            abc.acntspayslip_designation =designation.objects.get(id=vars.designation_id)
            abc.acntspayslip_user_id = user_registration.objects.get(id=id)
            
            abc.save()
            msg_success="salary added"
            return render(request, 'software_training/training/account/accounts_payment_salary.html',{'vars':vars,'mem' : mem,'msg_success':msg_success})
...
def accounts_report_issue(request):
    if 'acc_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            vars.save()
            msg_success="issue reported successfully"
            return render(request, 'software_training/training/account/account_report_issue.html',{'mem':mem,'msg_success':msg_success})
...
def accounts_add_bank_acnt(request, id):
    if 'acc_id' in request.session:  
        ...
        if request.method == 'POST': 
            ...
            vars.save() 
            msg_success = "account added"
            return render(request, 'software_training/training/account/accounts_add_bank_acnt.html',{'mem':mem, 'mem1':mem1,'msg_success':msg_success,})
...
def accounts_expenses_viewEdit_Update(request, id):
    if 'acc_id' in request.session:
        ...
        if request.method == 'POST':            
            ...
            emps.save() 
            msg_success=" transaction updated"
            return render(request,'software_training/training/account/accounts_expenses_viewEdit.html',{'mem':mem,'msg_success':msg_success,})
...
def accounts_expense_newTransaction(request):
    if 'acc_id' in request.session:
        ...
        if request.method == 'POST':
            ...
            mem1.save()
            msg_success="new transaction created"
            return render(request, 'software_training/training/account/accounts_expense_newTransaction.html',{'mem':mem,'msg_success':msg_success,})
        else:
...
def accounts_paydetails(request,id):
     if 'acc_id' in request.session:
        ...
        acc = acntspayslip.objects.filter(acntspayslip_user_id=id)
        names = acntspayslip.objects.all()
